...
func TestNewDate(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			d, err := scalarutils.NewDate(tc.day, tc.month, tc.year)
			if tc.expectError {
				assert.NotNil(t, err)
				assert.Nil(t, d)
...
func TestURL_UnmarshalGQL(t *testing.T) {
	...
	tests := []struct {
		name    string
		args    args
		want    scalarutils.URL
		wantErr bool
	}{
		{
			name:    "good case-valid url",
			args:    args{v: "http://www.example.com/index.html"},
			want:    scalarutils.URL("http://www.example.com/index.html"),
			wantErr: false,
		},
		{
...
func TestURL_MarshalGQL(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "good case",
			sc:    scalarutils.URL(goodLink),
			wantW: `"` + goodLink + `"`,
		},
		{
			name:  "invalid URL",
			sc:    scalarutils.URL(badLink),
			wantW: `"` + errorMessage + `"`,
		},
	}
	for _, tt := range tests {
...
func TestDateTime_Time(t *testing.T) {
	...
	validTime, err := time.Parse(scalarutils.DateTimeFormatLayout, timeStr)
	assert.Nil(t, err)

	tests := []struct {
		name string
		sc   scalarutils.DateTime
		want time.Time
	}{
		{
			name: "good case",
			sc:   scalarutils.DateTime(timeStr),
			want: validTime,
		},
		{
			name: "bad case",
			sc:   scalarutils.DateTime("this is not a valid date string"),
			want: time.Unix(0, 0),
		},
	}
	for _, tt := range tests {
...
func TestBase64Binary_UnmarshalGQL(t *testing.T) {
	var sc scalarutils.Base64Binary
	type args struct {
		v interface{}
	}
	tests := []struct {
		name    string
		sc      *scalarutils.Base64Binary
		args    args
		wantErr bool
	}{
		{
...
func TestBase64Binary_MarshalGQL(t *testing.T) {
	sc := scalarutils.Base64Binary("Y2FydG9vbg==")
	tests := []struct {
		name  string
		sc    scalarutils.Base64Binary
		wantW string
...
func TestCanonical_UnmarshalGQL(t *testing.T) {
	var sc scalarutils.Canonical
	type args struct {
		v interface{}
	}
	tests := []struct {
		name    string
		sc      *scalarutils.Canonical
		args    args
		wantErr bool
	}{
		{
...
func TestCanonical_MarshalGQL(t *testing.T) {
	sc := scalarutils.Canonical("http://hl7.org/fhir/ValueSet/my-valueset|0.8")
	tests := []struct {
		name  string
		sc    scalarutils.Canonical
		wantW string
...
func TestDateTime_MarshalGQL(t *testing.T) {
	sc := scalarutils.DateTime("2020-01-01")
	tests := []struct {
		name  string
		sc    scalarutils.DateTime
		wantW string
...
func TestInstant_UnmarshalGQL(t *testing.T) {
	var sc scalarutils.Instant
	type args struct {
		v interface{}
	}
	tests := []struct {
		name    string
		sc      *scalarutils.Instant
		args    args
		wantErr bool
	}{
		{
...
func TestInstant_MarshalGQL(t *testing.T) {
	sc := scalarutils.Instant("2020-01-01")
	tests := []struct {
		name  string
		sc    scalarutils.Instant
		wantW string
...
func TestMarkdown_UnmarshalGQL(t *testing.T) {
	var sc scalarutils.Markdown
	type args struct {
		v interface{}
	}
	tests := []struct {
		name    string
		sc      *scalarutils.Markdown
		args    args
		wantErr bool
	}{
		{
...
func TestMarkdown_MarshalGQL(t *testing.T) {
	sc := scalarutils.Markdown("this is Markdown")
	tests := []struct {
		name  string
		sc    scalarutils.Markdown
		wantW string
...
func TestUUID_MarshalGQL(t *testing.T) {
	...
	sc := scalarutils.UUID(randomUUID)
	tests := []struct {
		name  string
		sc    scalarutils.UUID
		wantW string
...
func TestXHTML_UnmarshalGQL(t *testing.T) {
	var sc scalarutils.XHTML
	type args struct {
		v interface{}
	}
	tests := []struct {
		name    string
		sc      *scalarutils.XHTML
		args    args
		wantErr bool
	}{
		{
